...
plot_all_backgrounds_separately = function(dat, dat.background, column.names.to.normalize) {
    ...
    for (journal in journals) {
    	...
    	plot_background(dat[inJournal, ], dat.background[[journal]], column.names.to.normalize, title=journal, yrange, colour=rainbow(length(journals))[i])
    	dev.off()
    }
}

plot_all_backgrounds_overlay = function(dat, dat.background, column.names.to.normalize) {
    # Get ranges so all journals can be plotted on the same axes
    journals = names(dat.background)
    yrange = get_ranges(dat, column.names.to.normalize)
