...
dat.eventcounts.nowos = read.csv("../data/derived/event_counts_altmetrics_cleaned.txt", header=TRUE, sep=",", stringsAsFactors=FALSE)
#load("../data/derived/eventcounts_preprocessed.RData")

# include ISI WoS data 
dat.wos = read.csv("../data/derived/event_counts_wos_extracted.txt", header=TRUE, sep="\t", stringsAsFactors=FALSE)
