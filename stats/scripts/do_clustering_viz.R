...
do_factor_analysis_viz = function() {
    clusterColumns = c("htmlDownloadsCount","plosCommentCount","mendeleyReadersCount","wosCountThru2011","f1000Factor","wikipediaCites")
    dat.for.cluster = subset(dat.research.norm.transform, journal=="pone")
    dat.for.cluster$altmetricsCombo = dat.for.cluster$facebookShareCount + dat.for.cluster$deliciousCount + dat.for.cluster$almBlogsCount + dat.for.cluster$backtweetsCount
